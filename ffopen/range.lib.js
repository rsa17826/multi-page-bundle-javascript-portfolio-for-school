// ==UserScript==
// @name         lib:range
// @version      1
// @description  adds range function to allow for i of range(0, 10, step:optional)
// @license      GPLv3
// @run-at       document-start
// @author       You
// @match        *://*/*
// @icon         data:image/png;base64,
// @grant        none
// @namespace https://greasyfork.org/users/1184528
// @downloadURL https://update.greasyfork.org/scripts/491568/lib%3Arange.user.js
// @updateURL https://update.greasyfork.org/scripts/491568/lib%3Arange.meta.js
// ==/UserScript==
;(() => {
  const a = loadlib("allfuncs")
  loadlib("libloader").savelib("range", range)
  function range(n1, n2, step = String(n1).match(/\.(.*)/)?.[1]?.length ?? 0) {
    if (n2 == undefined) {
      n2 = n1
      n1 = 0
    }
    if (!(String(n1).endsWith("0") && String(n1).length > 7)) n1 = Number(n1)
    if (!(String(n2).endsWith("0") && String(n2).length > 7)) n2 = Number(n2)
    var places = step
    step = 1 / Math.pow(10, step)
    var arr = []
    var i = n1
    var namedrange = new (class range {
      constructor() {
        this.from = n1
        this.to = n2
      }
    })()
    Object.getPrototypeOf(arr).name = "asd"
    return new Proxy(namedrange, {
      get(_obj, prop) {
        if (prop !== Symbol.iterator) return Reflect.get(namedrange, prop)
        return function* () {
          while (true) {
            i += step
            i = Number(a(i).toplaces(undefined, places).val)
            yield i
            if (i > n2) break
          }
        }
      },
    })
  }
})()
