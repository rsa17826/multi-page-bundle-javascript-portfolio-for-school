// ==UserScript==
// @name         lib:newelem
// @version      2
// @description  none
// @license      GPLv3
// @run-at       document-start
// @author       rssaromeo
// @match        *://*/*
// @tag          lib
// @exclude      /livereload.net\/files\/ffopen\/index.html$/
// @icon         data:image/png;base64,
// @grant        none
// @namespace https://greasyfork.org/users/1184528
// @downloadURL https://update.greasyfork.org/scripts/491564/lib%3Anewelem.user.js
// @updateURL https://update.greasyfork.org/scripts/491564/lib%3Anewelem.meta.js
// ==/UserScript==
;(() => {
  const a = loadlib("allfuncs")
  loadlib("libloader").savelib(
    "newelem",
    function (type, data = {}, inside = []) {
      var parent = a(null).createelem(type, data).val
      inside.forEach((elem) => {
        parent.appendChild(elem)
      })
      return parent
    }
  )
})()
