...
use allfuncs.new(function_name, function, folders)`
                )
              },
              deleteProperty(_a, s) {
                throw new Error(
    ...
    function (
      ...
      function repl(elem) {
        if (elem.shadowRoot) {
          ...
          a("*").qsa(elem.shadowRoot).foreach(repl)
          watch(elem.shadowRoot)
        }
        if (elem.children) [...elem.children].forEach(repl)
        if (!elem.matches) return
        if (ignoretags.includes(elem.tagName)) return
        if (elem.matches(selector)) func(elem)
      }
    },
    "elem"
  )
